...
def main() -> None:
    ...
    if kv_layout != 0:
        raise RuntimeError("int8 KV path supports kv_layout=0 only")
    ...
    cache = init_kv_cache(num_kv_heads, head_dim, args.pos + args.tokens, kv_layout, device, torch.float32, kv_quant="int8")
    ext = load_extension()
    ...
    out_kernel = ext.megakernel_forward_seq_persistent_int8kv(
        xseq,
        weights["rms_attn"],
        weights["rms_ffn"],
        weights["w_qkv"],
    ...
    kv_ref = {"k": torch.zeros(args.pos + args.tokens, num_kv_heads * head_dim, device=device),
              "v": torch.zeros(args.pos + args.tokens, num_kv_heads * head_dim, device=device)}
    out_ref = []
